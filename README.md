...
Claudio is an installable PWA — it launches full-screen with its own icon, no
browser chrome. It needs to be served over **HTTPS** (or `localhost`); installing
from a phone means the app must be reachable from your phone (same Wi-Fi tunnel or a
deployed URL).

- **iOS (Safari):** open the app → Share → **Add to Home Screen**.
- **Android (Chrome):** open the app → menu (⋮) → **Install app** / **Add to Home screen**
  (Chrome also shows an install prompt automatically once the service worker registers).

The app icon, splash colors, and standalone display come from `src/app/manifest.ts`,
the Apple meta tags in `src/app/layout.tsx`, and the service worker in `public/sw.js`.
Icons are generated by `npm run gen:icons` (a dependency-free PNG generator in
`scripts/gen-icons.mjs`) into `public/icon-192.png`, `icon-512.png`, and
`apple-touch-icon.png`.

> Tip: to test install locally from your phone, expose the dev server over HTTPS
> (e.g. a tunnel like `cloudflared`/`ngrok`) or deploy it (Vercel + Postgres), then
> open that URL on the phone and Add to Home Screen.
